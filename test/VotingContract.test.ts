...
describe("VotingContract", function () {
  ...
  describe("Voting Session", function () {
    ...
    it("Should revert if session does not exist", async function () {
      ...
      await expect(votingContract.getSessionCreator(1)).to.be.revertedWith(
        "Session does not exist",
      );
    });
  ...
  describe("Candidate Management", function () {
    ...
    it("Should fail to add a candidate to an active session", async function () {
      ...
      await expect(
        votingContract.addCandidate(0, "Candidate B"),
      ).to.be.revertedWith(
        "Candidates cannot be added during the voting period.",
      );
    });
  });
  ...
  describe("Voting Functionality", function () {
    ...
    it("Should fail if voting outside the voting period", async function () {
      ...
      await expect(votingContract.connect(user1).vote(0, 0)).to.be.revertedWith(
    ...
    it("Should fail to start voting window without candidates", async function () {
      const { votingContract, user1 } = await deployVotingFixture();
